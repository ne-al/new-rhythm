/*
  ...
  - Credits goes to the creator of blackhole music app
  - for this jiosaavn_handler package
...
import 'package:jiosaavn_handler/src/helper/format.dart';
import 'package:logging/logging.dart';

class Api {
  List preferredLanguages = ['hindi', 'punjabi', 'bhojpuri', 'english'];
  ...
  Future<Response> getResponse(
    ...
    if (useProxy) {
      const String proxyIP = '103.47.67.134';
  ...
  Future<List<Map<String, dynamic>>> fetchSearchResults(
    ...
    if (res.statusCode == 200) {
      ...
      searchedSongList = (await Api().fetchSongSearchResults(
  ...
  Future<List> fetchMultipleSongDetails(String songId) async {
    final String params = 'pids=$songId&${endpoints["songDetails"]}';
    try {
      final res = await getResponse(params);
      if (res.statusCode == 200) {
        final Map data = json.decode(res.body) as Map;
        return await FormatResponse.formatSongsInList(data['songs']);
      }
    } catch (e) {
      Logger.root.severe('Error in fetchSongDetails: $e');
    }
